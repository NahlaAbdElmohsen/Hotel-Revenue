...
select arrival_date_year,arrival_date_month,ROUND(sum(((stays_in_week_nights+stays_in_weekend_nights)*adr)*(1-Discount)-Cost),1) as profit
...
select top(10)arrival_date_year,hotel,mc.meal,ms.market_segment,ROUND(sum(((stays_in_week_nights+stays_in_weekend_nights)*adr)*(1-Discount)-Cost),1) as profit
from hotel_revn as h
join meal_cost as mc
on mc.meal=h.meal
join market_segment as ms
...
select arrival_date_year,round(sum(case when is_canceled=0 then stays_in_week_nights*adr end),1) as revenue_in_regular_days,round(sum(case when is_canceled=0 then stays_in_weekend_nights*adr end),1) as revenue_in_public_holidays
from hotel_revn
group by arrival_date_year
...
select hotel,market_segment,meal,sum(case when is_canceled=0 then stays_in_week_nights+stays_in_weekend_nights end) as number_of_nights_booked,round(sum((stays_in_week_nights+stays_in_weekend_nights)*adr),1) as total_revenue
from hotel_revn
...
select arrival_date_year,hotel,ROUND(sum(((stays_in_week_nights+stays_in_weekend_nights)*adr)*(1-Discount)-Cost),1) as profit,sum((stays_in_week_nights+stays_in_weekend_nights)) as total_nights_booked
from hotel_revn as h
join meal_cost as mc
on h.meal=mc.meal
join market_segment as ms
...
select arrival_date_year,country as country_code,count(case when is_canceled=0 then 1 end) as #Visitors,round(sum((stays_in_week_nights+stays_in_weekend_nights)*adr),1) as total_revenue
from hotel_revn
group by country,arrival_date_year
order by #Visitors desc

--Q9: what is the most popular meal and room type per each year?
select arrival_date_year,meal_cost.meal,assigned_room_type,COUNT(case when is_canceled=0 then 1 end) as #booking,round(sum(case when is_canceled=0 then ((stays_in_week_nights+stays_in_weekend_nights)*adr)end),1) as total_revenue
from hotel_revn
join meal_cost
...
--without years
select meal_cost.meal,assigned_room_type,COUNT(case when is_canceled=0 then 1 end) as #booking,round(sum(case when is_canceled=0 then ((stays_in_week_nights+stays_in_weekend_nights)*adr)end),1) as total_revenue
from hotel_revn
join meal_cost
on meal_cost.meal=hotel_revn.meal
